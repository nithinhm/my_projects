...
data_url = 'streamlit_project/Motor_Vehicle_Collisions_-_Crashes.csv'
...
st.header('Top 5 dangerous streets by affected type of commuter')
select = st.selectbox('Affected type of commuter', ['Pedestrians', 'Cyclists', 'Motorists'])

if select == 'Pedestrians':
    st.write(
        odata.query('injured_pedestrians >= 1')[['on_street_name', 'injured_pedestrians']].sort_values(
    ...
    st.write(
        odata.query('injured_cyclists >= 1')[['on_street_name', 'injured_cyclists']].sort_values(
            by=['injured_cyclists'],
            ascending=False
        ).dropna(how='any')[:5]
    )
    ...
    st.write(
        odata.query('injured_motorists >= 1')[['on_street_name', 'injured_motorists']].sort_values(
